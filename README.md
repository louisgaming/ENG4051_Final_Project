...
# The Idea
This project implements a solution for snacks and beverages dispensers, where an RFID Reader (RC522 module) indentifies a previously registered user for automatic balance deduction. The user can then view the quantity — measured by a scale — and the corresponding price of the order on an e-paper display. All system dynamics are managed by an ESP32-3S, with MQTT handling message transmission, Beekeeper Studio providing data storage, and Node-RED serving as the intermediary platform.  
